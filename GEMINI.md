...
- **타일 등록 UI 개선**
    - 타일 제목 입력 필드를 `textarea`로 변경하고 최대 100자까지 입력 가능하도록 확장했습니다.
    - 입력 필드 아래에 현재 글자 수와 최대 글자 수를 표시하는 카운터 `(0/100)`를 추가했습니다.
    - 글자 수 제한에 도달하면 카운터 텍스트가 빨간색으로 변경되어 시각적으로 알립니다.
    - 백엔드 유효성 검사(`src/routes/api/tiles/+server.ts`)도 100자로 업데이트했습니다.
    - 타일 컴포넌트(`src/lib/components/Tile.svelte`)에서 제목이 표시되는 줄 수를 3줄에서 5줄로 늘려 긴 제목도 더 잘 보이도록 개선했습니다.
    - 다국어 지원 파일(`src/lib/locales/ko.json`, `src/lib/locales/en.json`)에서 '이벤트 제목' 레이블을 '이벤트'로 간결하게 변경했습니다.
