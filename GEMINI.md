...
| --- | --- | --- | --- |
| `/api/tiles` | `GET` | 렌더링에 필요한 모든 타일 목록을 조회합니다. | 명세와 달리, `target_date`가 7일 이상 지난 타일은 제외하여 클라이언트의 부담을 줄입니다. |
...
- **추가된 주요 기능 (명세서 외)**:
    ...
    - **공유 기능**: '공유' 버튼을 통해 특정 타일로 바로 이동할 수 있는 URL을 클립보드에 복사할 수 있습니다.
...
| --- | --- | --- | --- |
| **데이터 모델** | `id`, `title`, `target_date`, `coord_x`, `coord_y`, `created_at` | 명세서 필드 + `color`, `likes` | **기능 확장**: 사용자의 표현(색상)과 상호작용(좋아요)을 강화하기 위해 필드가 확장되었습니다. |
...
| **추가 기능** | - | 좋아요, 공유, 미니맵, 색상 선택, 랜덤 배치 | **가치 증대**: 초기 기획에는 없었지만, 서비스의 재미와 사용성을 크게 증대시키는 기능들이 다수 추가되었습니다. |
